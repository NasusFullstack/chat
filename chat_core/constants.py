...
# 비밀번호 인증이 실패했을 때 보여줄 안내.
# 서버가 주는 "SASL authentication failed" 한 줄로는 무엇을 해야 할지 알 수 없다.
# 가장 흔한 이유는 그 이름을 서버에 등록하지 않은 것이다(실측으로 확인)
SASL_FAILED_HELP = (
    "비밀번호 인증에 실패했습니다. 이 이름으로 만든 서버 계정이 없거나 비밀번호가 다릅니다. "
    "계정을 만들려면 채팅창에 /register 비밀번호 이메일 을 입력하세요. "
    "(계정 없이 쓰려면 로그인할 때 비밀번호 칸을 비워두면 됩니다)")
