...
| | |
|---|---|
| **두 가지 프로토콜** | 직접 만든 JSON 프로토콜(계정·채널·비밀번호, TLS)과 **실제 IRC 서버**(RFC 1459)를 같은 화면에서 지원 |
...
| **아이콘 · 닉네임 · @호출** | IRC에는 없는 기능을 CTCP 프레임으로 우리 클라이언트끼리 주고받음 |
...
gui/
  pages/         화면 - 부품을 조립만 함
  components/    채널목록 · 참여자 · 입력줄 · 메시지 · 대화목록
...
**회귀 테스트 48개**가 실제 서버를 띄워서 돕니다(`python tests/run_all.py`).
...
`Python 3.13` `PySide6(Qt6)` `asyncio` `TLS` `IRC(RFC 1459 · IRCv3)` `PyInstaller` `Inno Setup` `GitHub Actions`
