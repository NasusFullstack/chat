...
# 배지와 목록 오른쪽 끝 사이 여백. 끝에 바짝 붙이되 테두리에 닿지는 않을 만큼만
BADGE_RIGHT_MARGIN = 2
# 이름과 배지 사이 최소 간격
BADGE_LEFT_GAP = 6
...
class ClientBadgeDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        # **원래 줄 넓이를 따로 떠둔다.** option.rect를 줄여서 글자를 그리게 한 뒤 그
        # 사각형으로 배지 자리를 계산하면, 배지가 줄어든 만큼 왼쪽으로 밀려 이름과
        # 겹친다(실제로 그렇게 보였음). 줄이기 전 값이 배지 자리의 기준이다
        full = QRect(option.rect)
        reserved = CLIENT_BADGE_PX * 2 + BADGE_GAP + BADGE_RIGHT_MARGIN + BADGE_LEFT_GAP
        option.rect = QRect(full.left(), full.top(),
                            max(0, full.width() - reserved), full.height())
        super().paint(painter, option, index)
        option.rect = full

        marker_x = full.right() - BADGE_RIGHT_MARGIN - CLIENT_BADGE_PX + 1
