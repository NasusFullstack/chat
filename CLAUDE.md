...
**아직 값이 남은 곳은 `main_window.py`(665)** 하나다. 접속 절차(약 190줄)가 창 조립과 같이
있다. 다만 이걸 빼내려면 컨트롤러가 소켓·세션·로그인 화면·채널 화면을 모두 알아야 해서
인터페이스가 넓어진다 - 그래서 "넓은 인터페이스를 만들면서까지 뺄 값이 있는가"를 먼저
따져보고 시작할 것. 지금은 검사(login_request.py)와 재접속 정책(reconnect.py)이 이미
빠져 있어 급하지 않다.
