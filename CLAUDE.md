...
만지면 다시 벌어진다("됐다 안 됐다 한다"). `_ChatLogContent.heightForWidth()`가 레이아웃의
`sizeHint().height()`를 돌려주도록 해서 맞췄다(말풍선 라벨에는 `set_wrap_width()`로 최대폭을
이미 지정해두므로 그 sizeHint가 지금 폭 기준 정답이다).
